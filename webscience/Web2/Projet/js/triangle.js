class Joueur {
    constructor(x, y, l, h, sprites, shield, scale) {
        this.x = x;
        this.y = y;
        this.scale = scale || 1;
        this.l = l * this.scale;
    ...
    draw(ctx) {
        ...
        if (this.state == entityStates.alive) {
            ...
            if (timeLeft <= this.shield_cooldown * 1000) {
                ...
                ctx.fillStyle = "green";
                ctx.fillRect(this.x + offset / 2, this.y - this.h / 2, (this.l - offset / 2) * timeLeft / (this.shield_cooldown * 1000), 5);
            }

            if (this.shield_activated == true) {
                this.shield_sprite[0].draw(ctx, this.x - this.l / 5, this.y - this.h * 1.2, 0.4);
            }
        }
